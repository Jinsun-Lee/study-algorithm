...
int dy[4] = {0,1,0,-1}; // 남쪽, 동쪽, 북쪽, 서쪽 순서로 / 북쪽을 시작으로 시계방향
int dx[4] = {1,0,-1,0};

int board1[10][10]; // 입력으로 주어지는 사무실의 모양 저장
...
// (x,y)에서 dir 방향으로 진행하면서 벽을 만나기 전까지의 모든 빈칸을 7로 바꿈
...
int main(void) {
  ...
  cin >> n >> m;
  // cctv가 1개 이상이라는 말이 없음 = 0개일 수도 있음 -> ans을 0x7f7f7f7f같은 값으로 두는 것보다 안전함
  ...
  for(int tmp = 0; tmp < (1<<(2*cctv.size())); tmp++){ // 카메라가 3대였을 떈, 0 ~ 63
    ...
    for(int i = 0; i < cctv.size(); i++){ // 카메라가 3대였을 떈, 0 ~ 3
      int dir = brute % 4; // 1의 자리수 = 각 자리수가 방향
      brute /= 4;          // 다음 자리수 / tmp의 값을 4진법으로 바꾸기 위해 나눔
